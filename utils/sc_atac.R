...
dap_volcano_plot <- function(se.object, 
  ...
  ggsave(paste0(plots.dir, "scatac_volcano_", "cluster_", cluster.name, "_", ident.1, "_vs_", ident.2, ".jpeg" ), plot = p, width = 8, height = 8)
...
FindTopDAPGenes <- function(da.peaks.all, 
  ...
  if (nrow(da.peaks.top) > 0) {
    ...
    if (nrow(closest.gene.dap) > 0){
      ...
      ggsave(paste0(plots.dir, "scatac_closest_genes_dap_gex_", "cluster_", cluster.name, "_", group, ".jpeg"), plot = p, width = 8, height = 8)
    ...
    for (v in 1:nrow(closest.gene.dap)){
      ...
      if(closest.gene.dap$gene_name[v] %in% rownames(se.integrated.atac@assays[["RNA"]]@features) & 
        ...
        ggsave(paste0(plots.dir, "scatac_closest_genes_dap_coverage_", "cluster_", cluster.name, "_", closest.gene.dap$gene_name[v], ".", plots.format), 
               ...
               width = 8, 
               height = 8)
        ggsave(paste0(plots.dir, "scatac_closest_genes_dap_coverage_", "cluster_", cluster.name, "_", closest.gene.dap$gene_name[v],".jpeg"), 
               plot = p,
               width = 8, 
               height = 8)
      }
...
TopMotifFootprints <- function(se.integrated.atac, enriched.motifs, cluster.name, group, plots.dir) {
  if (nrow(enriched.motifs) > 0){
    ...
    ggsave(paste0(plots.dir, "scatac_motif_fp_", "cluster_", cluster.name, "_", group, ".jpeg"), plot = p, width = 8, height = 8)
  }
}
